...
public class Plansza {
    ...
    boolean setPole(int x, int y, Pole pole) {
        if (x < SIZE || y < SIZE) {
            plansza[x][y].setStatus(pole.getStatus());
            return true;
        }
        return false;
    }
    ...
    public Texture getPoleTexture(int y, int x) {
        if (getPole(x, y).getStatus().equals(T))
            return new Texture("effect/marked.png");
        if (getPole(x, y).getStatus().equals(P))
            return new Texture("effect/O.png");
        if (getPole(x, y).getStatus().equals(B))
            return new Texture("effect/water.png");
        if ((checkLeft(x, y, "statek") || checkLeft(x, y, T)) && (checkRight(x, y, "statek") || checkRight(x, y, T)))
            return new Texture("ship/midH.png");
        if (!checkLeft(x, y, "statek") && (checkRight(x, y, "statek")|| checkRight(x, y, T)))
            return new Texture("ship/rearH.png");
        if ((checkLeft(x, y, "statek")|| checkLeft(x, y, T)) && !checkRight(x, y, "statek"))
            return new Texture("ship/frontH.png");
        if ((checkUpper(x, y, "statek")||checkUpper(x, y, T)) && (checkLower(x, y, "statek")||checkLower(x, y, T)))
            return new Texture("ship/midV.png");
        if (!checkUpper(x, y, "statek") && (checkLower(x, y, "statek")||checkLower(x, y, T)))
            return new Texture("ship/rearV.png");
        if ((checkUpper(x, y, T)||checkUpper(x, y, "statek")) && !checkLower(x, y, "statek"))
            return new Texture("ship/frontV.png");
        return new Texture("ship/single.png");
    }
